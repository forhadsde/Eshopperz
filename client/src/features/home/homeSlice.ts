...
export const fetchProductAsync = createAsyncThunk<Product, number>(
    ...
    async (productId, thunkAPI) => {
        try {
            const product = await agent.home.details(productId);
            return product;
        } catch (error: any) {
            return thunkAPI.rejectWithValue({ error: error.data })
        }
...
export const homeSlice = createSlice({
    name: 'home',
    initialState: productsAdapter.getInitialState<homeState>({
...
export const {setProductParams, resetProductParams, setMetaData, setPageNumber} = homeSlice.actions;
